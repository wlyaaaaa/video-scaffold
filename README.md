...
本地视频制作工具链：Fish Audio 配音与原生时间戳、按需复用 ChineseASR 本地对齐、确定性 SVG 动画、有声审阅、FFmpeg 合成、封面、章节与可选字幕。默认生产规格为 **3840×2160、60fps、AV1 NVENC**。不替用户决定选题，也不自动投稿。
...
初始化只接受空目标目录，不复制旧工程素材、音频、成片或密钥；先暂存完整模板再移入目标。进入新目录后运行 doctor；test 是维护回归入口，不是每次制作的必经步骤。解释器优先级为 VIDEO_PYTHON、项目 .venv、机器适配、py -3.11、系统 python。依赖锁记录本项目的实际依赖闭包，不包含个人全局环境的其它软件。
...
```powershell
pwsh -File .\run.ps1 preview --open
pwsh -File .\run.ps1 cleanup --dry-run
```

有声预览支持词语跳转、时间拖动、场景切换和暂停保留画面。cue 编辑器导出修改片段，不直接覆盖原件；保存后重建才能生效。serve 只在回环临时端口提供预览依赖，Ctrl+C 停止；不带 --open 的 preview 只生成文件。
...
持续集成包含 Windows/Linux 逻辑测试及 Linux CPU 隔离短片，不接触个人数据或云端账户。核心安装不含 Whisper/CTranslate2/CUDA 模型库；Fish 原生时间戳为新旁白的默认来源。ChineseASR 在其独立运行时按需对齐，不做默认双模型转写；解析测试仍不能代替真实账户/声线验收。

完整用法、恢复语义和验证边界见 [当前工作流](docs/WORKFLOW.md)，SVG 方法见 [创作指南](docs/AUTHORING.md)，进阶视觉组件见 [高级动效](docs/ADVANCED_FX.md)。项目规则入口为 AGENTS.md。

旧定制 builder 和封面源码已退出活动树，历史版本保留在 Git；不会清理旧工程原件。唯一生产入口是 run.ps1 / pipeline.workflow。

## 语音联动与旧工程

默认 `VIDEO_TIMING_SOURCE=auto`：先复用来源与内容仍匹配的已有词轴；需要生成词轴时优先使用该音频的 Fish 原生时间戳，否则调用本机登记的 ChineseASR 对齐入口。已存在但损坏的时间戳会报错，不静默掩盖。

```powershell
# 给同一段已接受的音频重新做本地对齐，不重配音
pwsh -File .\run.ps1 timing --source chinese-asr --force
# 单独生成/刷新场景时长；不运行语音识别
pwsh -File .\run.ps1 durations
```

旧配音 v1 来源记录及绑定有效的旧 Whisper 词轴可在 auto 模式继续读取；不伪造新来源，不因切换词轴强制 tts --force。需要重新计算时仅重建词轴及受影响画面。共享入口一次处理一个有文案的场景，当前对齐模型上限 300 秒，超长场景会要求按已审阅的分镜切分。

ChineseASR 解释器与源码根仅在被忽略的机器适配中登记。公开模板仍能只靠 Fish 原生时间戳出片；缺少共享 ASR 不影响该路线。仅重建背景时才额外安装 `requirements-background.txt`，不重建背景无需安装它。
