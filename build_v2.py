...
用法： python build_v2.py [scripts|tts|timing|build|preview|render|merge|cover|chapters|all]
...
注意：raw_audio/ 已是审过的配音，别再跑 `tts`/`all`（会重合成、改时长）；
日常迭代用 `build` → `preview` →（满意再）`render`。
"""
...
from pipeline import chapters as chapters_mod, preview as preview_mod
...
def s00_open():
    ...
    return (
        ...
        + L.kicker("游戏王 MASTER DUEL · 氪金指南", M, 520, delay=0.2)
...
def s02_anchor():
    return (
        ...
        + L.bar_race([
            ("官方日区 · 1000钻", 50.48, INK, "五十块", "¥"),
            ("我们 · 倒余额", 37.8, ACCENT, "三十七", "¥"),
        ...
        + L.image("Google大师决斗-1美刀优惠券.jpg", 2600, 1620, w=940, anim="wipe", cue="弃用", delay=0.5)
        + L.text("美区≈¥61，Google 券也不划算 → 弃用", 2600, 1985, lvl=4, cue="弃用", delay=0.0, opacity=1, fill=RED)
    )
...
def s04_xianyu():
    return (
        L.title("那，某鱼代充呢？", M, 440, lvl=1, kick="便宜的代价", cue="代充", delay=0.3)
        + L.num(280, M, 760, lvl=1, cue="苹果", delay=0.1, prefix="¥", suffix=" 苹果端", fill=RED)
        + L.strike(M-10, 745, 560, cue="退款", delay=2.0)
        + L.num(370, M, 940, lvl=1, cue="其他", delay=0.1, prefix="¥", suffix=" 其他端", fill=RED)
        + L.strike(M-10, 925, 560, cue="退款", delay=2.1)
...
def s10_planb():
    return (
        ...
        + L.chip("不卡你用初始号的免费钻抽卡", M, 1120, lvl=3, cue="免费赚", delay=0.0)
...
def s11_buffcfg():
    return (
        L.title("网易 BUFF 基础配置", M, 360, lvl=1, kick="手机端一键设置就行", cue="正题", delay=0.3)
...
def s12_pick():
    return (
        ...
        + L.image("五大适合出售的箱子.jpg", 300, 1520, h=440, anim="wipe", cue="看不懂", delay=0.4)
        + L.callout(["买入 ¥4.06", "BUFF 批量购买"], 760, 820, cue="买入", delay=0.6, lvl=1, fill=GOLD)
        ...
        + L.text("以后未必是热潮：汇率/箱价会变，看不懂就把截图发 AI 算", 760, 1620, lvl=4, cue="看不懂", delay=4.0, opacity=1, fill=ACCENT)
    )

S12 = "买什么？批量买 CS2 的[emphasis]热潮武器箱。BUFF 上买入，一个四块零六；挂到 Steam 市场，卖六块一毛八，也就是[emphasis]一百四十多日元，扣完税到手大概一百二十九，差价就是利润。为什么选它？你看这条售价中位线，整整一个月几乎是条[emphasis]水平线，价格稳，不怕你买进去就砸盘。当然，以后未必还是热潮——汇率和箱价都会变，长期折扣可能越来越低，看不懂，就把买入价和卖出价截图，[emphasis]发给 AI 算。"
...
def s13_routeA():
    return (
        ...
        + L.text("整个 BUFF 批量买箱，约 30 分钟就能搞定 500 块的货", M, 1700, lvl=4, cue="三十分钟", delay=0.0, opacity=1)
    )

S13 = "倒余额有两条路。先说路线 A，直购法，大多数人走这条：直接在 BUFF 买箱，发给日区小号，小号收到，挂上 Steam 市场卖掉，换回日元。[emphasis]最简单，买、发、卖，三步到位。补一句，整个 BUFF 批量买箱，大概[emphasis]三十分钟，就能搞定五百块的货。"
...
S14 = "路线 B，是给手上[emphasis]本来就有闲置余额的大号准备的，进阶玩法，大部分人用不上。操作有个细节：大号用你平时的浏览器登录；小号的交易链接，单独开个[emphasis]无痕窗口去取，别在一个窗口里来回切。这次我实测，BUFF 上花四百九十五块三毛二，买进[emphasis]一百二十二个热潮箱——我只掏这么点，是因为大号还剩三十九块余额顶着。"
...
def s15_phish():
    return (
        ...
        + L.pop("不脱离 BUFF，不点外链——骗子无从下手", M, 1620, lvl=2, cue="无从下手", delay=0.0, fill=ACCENT)
    )

S15 = "最后一道保险，也是资金安全最重要的一关，防 API 钓鱼。在 Steam 收发报价，[emphasis]只走网易 BUFF App 里的指引。任何卖家买家私发的外部网页、API 链接，一律不点；任何外站让你填 Steam 账号密码，一律不填。手机确认报价时，再核对一遍对方的注册时间和等级，跟 BUFF 对得上才点。一句话：不脱离 BUFF，不点外链，骗子就[emphasis]无从下手。"
...
S18 = "把整套流程压成一条线：第一天，改绑、开令牌，顺手白嫖初始号的免费钻试玩。第七天，充值卡首充，切到日元区。第十五天，市场解锁，绑 BUFF 倒余额。再等七天，第二十二天，出售到账，买光活动钻。全程，[emphasis]官方绿卡，安全解锁。"
...
S21 = "对了，那个活动的限时两千五百钻、三千八百日元，[emphasis]只剩三十九天了。而这套操作，二十二天就够——你要是有充值计划，[emphasis]完全来得及。每千钻折到三十六七块，整体七四八折，真的很划算。觉得有用，麻烦点赞、投币、收藏，一键三连，再点个关注。补一句：钻石分平台不互通，认准你充值的那个平台。我是电子雪貂饲养员，我们，下期见。"


SCENES = [
    ("s00", s00_open, S00),
    ("s01", s01_hook, S01), ("s02", s02_anchor, S02), ("s03", s03_table, S03),
...
CHAPTER_GROUPS = [
    (0,  "导言：36元直出千钻是真的吗？"),         # s00 + s01
    (2,  "账目比对：官方原价 vs 走线成本"),     # s02 + s03
    (4,  "代充黑幕：某鱼低价代充为什么会封号？"),       # s04
    (5,  "原理解析：为什么绕道购买CS2箱子？"),      # s05
    (6,  "账号准备：Steam日区激活与防回改"),       # s06 + s07 + s08
    (9,  "算清时间差：为什么新号需要等22天？"),      # s09 + s10
    (11, "实操教学：选货箱子与BUFF平台配置"),        # s11 + s12
    (13, "倒余额流程：直购与大号赠送双路线"),        # s13 + s14 + s14b
    (16, "避坑必看：防API交易钓鱼与市场变现"),        # s15 + s16
    (18, "成绩单结算：实测我们的省钱比例"),        # s17 + s18
    (20, "补充避坑：有偿钻过期机制与平台限制"),          # s19 + s20 + s21
]
...
def main():
    ...
    if stage in ("timing", "all"):
        dur_mod.build(); transcribe.transcribe_batch()
